  ...
  const handleSaveQuery = async () => {
    ...
    try {
      const result = await saveQuery({
        title: message.chart_recommendation?.title || 'Saved from Chat',
        sql: message.sql,
        connection_id: connectionId,
        folder_name: 'Uncategorized',
      });
      setSaveLabel(result.created ? '✅ Saved!' : '📌 Already saved');
        ...
        {!message.error && (
          <div style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            <button
              ...
              disabled={saving || !!saveLabel || !message.sql}
              style={{
                padding: '5px 12px', borderRadius: 6, border: `1px solid ${T.border}`,
                background: 'transparent',
                color: saveLabel ? T.green : T.text3,
                fontSize: '0.72rem', cursor: (saving || !!saveLabel || !message.sql) ? 'default' : 'pointer', fontFamily: T.fontBody,
                display: 'flex', alignItems: 'center', gap: 5, transition: 'all 0.2s',
              }}
              onMouseEnter={e => { if(!saving && !saveLabel && message.sql) { e.currentTarget.style.background = T.s2; e.currentTarget.style.color = T.text2; } }}
              onMouseLeave={e => { if(!saving && !saveLabel && message.sql) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = saveLabel ? T.green : T.text3; } }}
            >
              {saveLabel || (saving ? '⏳ Saving...' : '📌 Save Query')}
            </button>
            <button
